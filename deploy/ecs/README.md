...
Runs network-buoy as a single Fargate task with all 19 protocol listeners. Logs go to EFS (persisted) and CloudWatch (streaming).
...
2. Builds the image from `buoy/` and pushes it (tagged with current git SHA + `latest`)
...
- CPU: 2 vCPU / 4 GiB is comfortable. Drop to 1 vCPU / 2 GiB if cost matters — the honeypot is IO-bound and lightweight; LLM inference is the only CPU-heavy workload, and only if `LLM_ENABLED=true`.
