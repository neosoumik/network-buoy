...
# MSG_KEXINIT(20) payload: 1 byte msg type + 16 byte cookie + algorithm lists
...
def _read_packet(conn: socket.socket) -> bytes:
    """Read one SSH binary packet and return the payload."""
    header = b""
    while len(header) < 4:
    ...
    while remaining > 0:
        ...
        payload += chunk
        remaining -= len(chunk)
    # payload[0] = padding_len, payload[1] = message type, rest = data
    return payload[1:] if len(payload) > 1 else b""


def _make_packet(payload: bytes) -> bytes:
    # Minimal SSH packet: len(payload+1) padded to 8 bytes, 6 bytes padding
    padding = 6
...
def _userauth_failure() -> bytes:
    ...
    body = (
        bytes([_MSG_USERAUTH_FAILURE]) + struct.pack(">I", len(methods)) + methods + b"\x00"
...
def _extract_userauth(payload: bytes) -> tuple[str, str, str]:
    """Extract (username, service, method) from SSH_MSG_USERAUTH_REQUEST."""
    try:
...
def handle_ssh(conn: socket.socket) -> None:
    ...
    with conn:
        try:
            ...
            conn.recv(256)  # client banner

            # Key exchange
            conn.sendall(_make_packet(_KEXINIT))
            conn.recv(4096)  # client KEXINIT

            # Read packets until we see USERAUTH_REQUEST or give up
            attempts = 0
            for _ in range(20):
                ...
                if msg_type == _MSG_SERVICE_REQUEST:
                    ...
                    attempts += 1
                    if attempts < 2:
                        # Fail first attempt — real servers do this
                        conn.sendall(_userauth_failure())
                    else:
                        # Accept on second attempt with motd as banner
                ...
                # Ignore other packet types (KEX_ECDH_INIT, NEWKEYS, etc.)
